...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-100']:
            ...
            backbone = resnet18()
        elif p['train_db_name'] == 'isic':
            from models.resnet_isic import resnet18
            backbone = resnet18()
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] in ['isic']:
        from data.isic import ISIC
        dataset =ISIC(train=True, transform=transform)
    else:
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    elif p['val_db_name'] == 'tiny-imagenet':
        from data.tinyimagenet import TinyImageNet
    ...
    elif p['val_db_name'] in ['isic']:
        from data.isic import ISIC
        dataset =ISIC(train=False, transform=transform)
