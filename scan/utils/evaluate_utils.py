...
def contrastive_evaluate(val_loader, model, memory_bank):
    ...
    model.eval()
    # print(len(val_loader.dataset));cnt = 0
    for iter, batch in enumerate(val_loader):
        # print(cnt, len(batch['image']));cnt +=len(batch['image'])
        images = batch['image'].cuda(device=1,non_blocking=True)
        target = batch['target'].cuda(device=1,non_blocking=True)
