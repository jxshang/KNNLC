...
class TypiClust:
    ...
    def init_features_and_clusters(self, is_scan):
        ...
        logger.info(f'Clustering into {num_clusters} clustering. Scan clustering: {is_scan}')
        if is_scan:
            fname_dict = {'CIFAR10': f'../../results/cifar-10/scan/features_seed{self.seed}_clusters{num_clusters}.npy',
                          'CIFAR100': f'../../results/cifar-100/scan/features_seed{self.seed}_clusters{num_clusters}.npy',
                          'STL10': f'../../results/stl-10/scan/features_seed{self.seed}_clusters{num_clusters}.npy',
                          'TINYIMAGENET': f'../../results/tiny-imagenet/scan/features_seed{self.seed}_clusters{num_clusters}.npy',
                          }
            ...
            self.clusters = kmeans(self.features, num_clusters=num_clusters)#clustering labels
        logger.info(f'Finished clustering into {num_clusters} clusters.')
    ...
    def select_samples(self):
        ...
        logger.info(f'Finished the selection of {len(activeSet)} samples.')
