...
class ProbCover:
    ...
    def construct_graph(self, batch_size=500):
        ...
        """
        xs, ys, ds = [], [], []
        logger.info(f'Start constructing graph using delta={self.delta}')
        ...
        logger.info(f'Finished constructing graph using delta={self.delta}')
        logger.info(f'Graph contains {len(df)} edges.')
        return df
    ...
    def select_samples(self):
        ...
        """
        logger.info(f'Start selecting {self.budgetSize} samples.')
        selected = []
        ...
        for i in range(self.budgetSize):
            ...
            logger.info(f'Iteration is {i}.\tGraph has {len(cur_df)} edges.\tMax degree is {degrees.max()}.\tCoverage is {coverage:.3f}')
            cur = degrees.argmax()
            ...
            selected.append(cur)
        self.cover_nums = covered_samples.shape[0]
        assert len(selected) == self.budgetSize, 'added a different number of samples'
        activeSet = self.relevant_indices[selected]
        remainSet = np.array(sorted(list(set(self.uSet) - set(activeSet))))
        logger.info('covered samples num:{}'.format(self.cover_nums))
