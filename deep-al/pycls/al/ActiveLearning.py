...
class ActiveLearning:
    ...
    def sample_from_uSet(self, clf_model, lSet, uSet, trainDataset, supportingModels=None):
        ...
        if self.cfg.ACTIVE_LEARNING.SAMPLING_FN == "random":
        ...
        elif self.cfg.ACTIVE_LEARNING.SAMPLING_FN == "maxherding":
            activeSet, uSet = self.sampler.maxherding(budgetSize=self.cfg.ACTIVE_LEARNING.BUDGET_SIZE,lSet=lSet,uSet=uSet \
                ,model=clf_model,dataset=trainDataset)
            ...
            clf_model.penultimate_active = True
            activeSet, uSet = self.sampler.badge(self.cfg.ACTIVE_LEARNING.BUDGET_SIZE, \
                lSet, uSet, clf_model, trainDataset)
            clf_model.penultimate_active = False
        elif self.cfg.ACTIVE_LEARNING.SAMPLING_FN == "balque":
            activeSet, uSet = self.sampler.balque(self.cfg.ACTIVE_LEARNING.BUDGET_SIZE, \
                lSet, uSet, clf_model, trainDataset)
        else:
