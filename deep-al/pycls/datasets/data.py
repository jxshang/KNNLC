...
from pycls.datasets.isic import ISIC
...
class Data:
    ...
    def getPreprocessOps(self):
        ...
        if self.dataset in self.datasets_accepted:
            ...
            if self.dataset in ["CIFAR10", "CIFAR100", 'IMBALANCED_CIFAR10', 'IMBALANCED_CIFAR100']:
                ops = [transforms.RandomHorizontalFlip(p=0.5), transforms.RandomCrop(32, padding=4)]
                norm_mean = [0.485, 0.456, 0.406]
                norm_std = [0.229, 0.224, 0.225]
            elif self.dataset in ["CIFAR100", 'IMBALANCED_CIFAR100']:
                ops = [transforms.RandomHorizontalFlip(p=0.5), transforms.RandomCrop(32, padding=4)]
                norm_mean = [0.5071, 0.4867, 0.4408]
                norm_std = [0.2675, 0.2565, 0.2761]
            elif self.dataset == 'STL10':
                ops = [transforms.RandomCrop(96, padding=4)]
                norm_mean = [0.485, 0.456, 0.406]
                norm_std = [0.229, 0.224, 0.225]
            ...
            elif self.dataset in ["ISIC"]:
                ops = [transforms.RandomRotation(10), transforms.ColorJitter(brightness=0.15, contrast=0.1),
            ...
            if not self.eval_mode and (self.aug_method == 'simclr'):
                ops.insert(1, get_simclr_ops(input_shape=self.cfg.TRAIN.IM_SIZE))
            ...
            if self.eval_mode:
                if self.dataset in ["ISIC"]:
                    ops = [transforms.CenterCrop(224), transforms.ToTensor(), transforms.Normalize(norm_mean, norm_std)]
                else:
                    ops = [transforms.ToTensor(), transforms.Normalize(norm_mean, norm_std)]
            else:
                logger.info("Preprocess Operations Selected ==>{}".format(ops))
            return ops
        else:
    ...
    def getDataset(self, save_dir, isTrain=True, isDownload=False):
        ...
        if self.dataset == "CIFAR10":
            ...
            isic = ISIC(save_dir, train=isTrain, transform=preprocess_steps,  test_transform=test_preprocess_steps, only_features=only_features)
    ...
    def makeLUVSets(self, train_split_ratio, val_split_ratio, data, seed_id, save_dir):
        ...
        train_labels = torch.tensor(data.targets, dtype = torch.int64)

        num_classes = self.cfg.MODEL.NUM_CLASSES
        single_class_nums = int(train_split_ratio*n_dataPoints)//num_classes
        for lb in range(num_classes):
            cnt = 0; i = 0
            while cnt < single_class_nums:
                while(all_idx[i] == -1 or train_labels[all_idx[i]] != lb): i+=1
                lSet.append(all_idx[i]);cnt +=1
                all_idx[i] = -1
        all_idx = torch.tensor(all_idx, dtype = torch.int64)
        remain_idx = all_idx[all_idx != -1]
        val_cnt = int(val_split_ratio*n_dataPoints)
        valSet = remain_idx[0:val_cnt].tolist()
        uSet = remain_idx[val_cnt:remain_idx.shape[0]].tolist()
